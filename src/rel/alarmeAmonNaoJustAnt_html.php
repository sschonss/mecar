...
<body>
	...
	<div class="container">
		...
		<p class="titulo" align="center"><b>Relatório de Alarmes Não Justificados Células de Amônia</b></p>
			...
			<table id="tabela" width="100%" cellspacing="0" border="1">
				...
				<tbody>
					...
					$query = $pdo->query("SELECT dfn.data_name as dataName, case when dfn.data_name IN ('data_format_1') then 'SDVA-SM1-C001' when dfn.data_name IN ('data_format_2') then 'SDVA-SM1-C002'
						...
						WHERE NOT EXISTS (SELECT e.event_index FROM `cMT-0484_event` e WHERE e.event_index != je.event_index) AND ev.event_log_index >= '2621' AND ev.event_log_index <= '2666' ORDER BY ev.`trigger_time@timestamp` desc;");
